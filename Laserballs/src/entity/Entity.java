...
public abstract class Entity {
	...
	public boolean intersects(Entity e) {
		...
		float dx = getWidth() / 2 * 0.95f;
		float dy = getHeight() / 2 * 0.95f;
		float edx = e.getWidth() / 2 * 0.95f;
		float edy = e.getHeight() / 2 * 0.95f;
	...
	public Entity getIntersectingEntity(Class cls) {
		for(Entity e : getWorld().getEntitiesInWorld())
			if(e.getClass().equals(cls) && intersects(e))
				return e;
		return null;
	}
	
	public ArrayList<Entity> getIntersectingEntities(Class cls) {
